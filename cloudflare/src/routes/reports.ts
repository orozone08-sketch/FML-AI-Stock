...
import { REPORT_NAMES, REPORTS, ReportRepository, printableRows, toCsv, type ReportFilters } from "../reports";
...
async function render(c:any,name:string, extra:ReportFilters={}) {
  ...
  if (format) {
    ...
    if (format==="csv") return new Response(toCsv(result.rows),{headers:{"content-type":"text/csv; charset=utf-8","content-disposition":`attachment; filename="${filename(name)}.csv"`}});
    if (format!=="print" && format!=="pdf") return c.text("Unsupported format",400);
  }
  const printable=printableRows(result.rows);
  const more=result.hasMore && result.nextCursor ? `<p><a href="?cursor_date=${result.nextCursor.date}&cursor_id=${result.nextCursor.id}">Next page</a></p>`:"";
  return c.html(layout(definition.title,table(printable.headers.map(escapeHtml),printable.rows.map(row=>row.map(escapeHtml)))+more,user));
}
