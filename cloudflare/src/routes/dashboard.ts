...
dashboard.get("/", async (c) => {
  ...
  const cards = `<section class="cards"><article><h2>Current stock</h2><strong>${qty(scalar(0, "quantity"))}</strong></article><article><h2>Receivables</h2><strong>₹${money(scalar(1, "total"))}</strong><small>${scalar(1, "count")} open</small></article><article><h2>Payables</h2><strong>₹${money(scalar(2, "total"))}</strong><small>${scalar(2, "count")} open</small></article><article><h2>Sales</h2><strong>${scalar(3, "count")}</strong></article><article><h2>Purchases</h2><strong>${scalar(4, "count")}</strong></article></section>`;
