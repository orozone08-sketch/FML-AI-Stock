...
dashboard.get("/", async (c) => {
  ...
  const company=rows(12)[0]??{name:"All Companies",code:""};
  const receivable=scalar(results,1,"total"),payable=scalar(results,2,"total");
  ...
  const cards=`<form class="inline-filters" method="get"><label>From <input type="date" name="from" value="${escapeHtml(range.from)}"></label><label>To <input type="date" name="to" value="${escapeHtml(range.to)}"></label><button type="submit">Apply</button></form><section class="metric-grid"><article class="metric-card" data-animate-card><span>Inventory</span><h3>Current stock</h3><strong data-count-value>${qty(scalar(results,0,"quantity"))}</strong><small>₹${money(scalar(results,0,"value"))} FIFO value</small></article><article class="metric-card danger" data-animate-card><span>Customer</span><h3>Receivable balance</h3><strong>₹${money(receivable)}</strong><small>${scalar(results,1,"overdue")} due or overdue</small></article><article class="metric-card amber" data-animate-card><span>Supplier</span><h3>Payable balance</h3><strong>₹${money(payable)}</strong><small>Supplier and inter-company dues</small></article><article class="metric-card" data-animate-card><span>Inter-company</span><h3>Pending balance</h3><strong>₹${money(scalar(results,6,"total"))}</strong><small>Open inter-company ledger</small></article><article class="metric-card danger" data-animate-card><span>Stock alerts</span><h3>Low / out items</h3><strong data-count-value>${scalar(results,5,"count")}</strong><small>Evaluated by book and item</small></article></section>`;
  const trendRows=rows(7);
  ...
  const monthSales=`<section class="grid two"><div class="panel"><div class="panel-title"><h2>Month Sales</h2><a href="/reports/sales-by-type">View report</a></div>${dashboardTable(["Month","Sales","Purchases"],trendRows.map(row=>[escapeHtml(row.month),`₹${money(row.sales_paise)}`,`₹${money(row.purchases_paise)}`]),"No sales recorded this month.")}</div><div class="panel"><div class="panel-title"><h2>Stock Alerts</h2><a href="/reports/stock-alerts">View report</a></div>${dashboardTable(["Book","Item","Qty"],rows(10).map(row=>[escapeHtml(`${row.company} · ${row.stock_book}`),escapeHtml(`${row.item_code} - ${row.item}`),qty(row.quantity_milliunits)]),"No stock alerts.")}</div></section>`;
  const overdue=`<section class="grid two"><div class="panel"><div class="panel-title"><h2>Overdue Receivables</h2><a href="/finance/outstanding">Outstanding</a></div>${dashboardTable(["Company","Customer","Document","Balance"],rows(8).map(row=>[escapeHtml(row.company),escapeHtml(row.party),escapeHtml(row.document_number),`₹${money(row.balance_amount_paise)}`]),"No overdue receivables.")}</div><div class="panel"><div class="panel-title"><h2>Overdue Payables</h2><a href="/finance/outstanding">Outstanding</a></div>${dashboardTable(["Company","Supplier","Document","Balance"],rows(9).map(row=>[escapeHtml(row.company),escapeHtml(row.party),escapeHtml(row.document_number),`₹${money(row.balance_amount_paise)}`]),"No overdue payables.")}</div></section>`;
  const interCompany=`<section class="panel"><div class="panel-title"><h2>Pending Inter-company</h2><a href="/reports/inter-company">View report</a></div>${dashboardTable(["Owner","User","Item","Quantity","Balance","Due"],rows(11).map(row=>[escapeHtml(row.owner),escapeHtml(row.user),escapeHtml(row.item),qty(row.quantity_milliunits),`₹${money(row.balance_amount_paise)}`,escapeHtml(row.due_date)]),"No pending inter-company entries.")}</section>`;
  return c.html(layout("Dashboard",hero+cards+analytics+monthSales+overdue+interCompany,user,{subtitle:"Stock, outstanding, alerts, and inter-company control at a glance.",company:{name:String(company.name),code:String(company.code??"")},dueAlertCount:scalar(results,1,"overdue")+scalar(results,2,"overdue")}));
});
