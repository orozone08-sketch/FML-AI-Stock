...
async function customerDirectory(c:any,item:MasterConfig):Promise<Response>{
  ...
  const base=`WITH parties AS (SELECT 'customer' kind,id,code,name,contact_person,mobile,gst_number,city,active party_active,LOWER(code||' '||name||' '||COALESCE(contact_person,'')||' '||COALESCE(mobile,'')||' '||COALESCE(gst_number,'')||' '||COALESCE(city,'')) search_text FROM customers UNION ALL SELECT 'supplier',id,code,name,NULL,mobile,gst_number,NULL,active,LOWER(code||' '||name||' '||COALESCE(mobile,'')||' '||COALESCE(gst_number,'')) FROM suppliers), directory AS (SELECT p.*,GROUP_CONCAT(DISTINCT co.id) company_ids,GROUP_CONCAT(DISTINCT co.code) company_label FROM parties p LEFT JOIN companies co ON (p.kind='customer' AND (EXISTS(SELECT 1 FROM sales s WHERE s.customer_id=p.id AND s.company_id=co.id AND s.is_void=0) OR EXISTS(SELECT 1 FROM receivables r WHERE r.customer_id=p.id AND r.company_id=co.id) OR EXISTS(SELECT 1 FROM payments py WHERE py.customer_id=p.id AND py.company_id=co.id))) OR (p.kind='supplier' AND (EXISTS(SELECT 1 FROM purchases pu WHERE pu.supplier_id=p.id AND pu.company_id=co.id AND pu.is_void=0) OR EXISTS(SELECT 1 FROM payables pa WHERE pa.supplier_id=p.id AND pa.company_id=co.id) OR EXISTS(SELECT 1 FROM payments ps WHERE ps.supplier_id=p.id AND ps.company_id=co.id))) GROUP BY p.kind,p.id) `;
