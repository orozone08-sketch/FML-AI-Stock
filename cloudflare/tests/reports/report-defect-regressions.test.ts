...
describe("confirmed report/export defect regressions",()=>{
 it("uses the real inventory balance value column and retains zero-balance book/item combinations",()=>{expect(REPORTS["current-stock"]!.sql).toContain("b.ledger_value_paise,0) fifo_value_paise");expect(REPORTS["current-stock"]!.sql).toContain("CROSS JOIN items");expect(REPORTS["current-stock"]!.sql).not.toContain("b.fifo_value_paise");});
